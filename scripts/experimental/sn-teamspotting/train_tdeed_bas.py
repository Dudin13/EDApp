...
def main(args):
    ...
    classes = load_classes(os.path.join('data', args.dataset, 'class.txt'), event_team = args.event_team)
    joint_train_classes = None
    if args.joint_train != None:
        joint_train_classes = load_classes(os.path.join('data', args.joint_train['dataset'], 'class.txt'), event_team = args.event_team)

    if not args.only_test:
        # Get datasets train, validation (and validation for map -> Video dataset)
        _, _, train_data, val_data, val_data_frames = get_datasets(args)
